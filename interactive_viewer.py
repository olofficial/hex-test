...
def interactive_viewer(lines, show_control_points = False):
    ...
    widgets.interact(plotty, lines=widgets.fixed(lines), line_number=line_number, upper_lim=(1, len(lines), 1),  layout=widgets.Layout(width='50%'), show_control_points = show_control_points, continuous_update=True)

def interactive_viewer(lines, show_control_points = False):
    line_number = widgets.IntText(value=len(lines), description='Max Lines:', min=1, max=len(lines))
    widgets.interact(plotty, lines=widgets.fixed(lines), upper_lim=(1, len(lines), 1), line_number=line_number, layout=widgets.Layout(width='50%'), show_control_points = show_control_points, continuous_update=True)